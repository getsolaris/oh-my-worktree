...
- **TUI 모드** — 인터랙티브 터미널 UI (`oml`)
- **CLI 모드** — 스크립트에서 사용 가능한 커맨드 (`oml add`, `oml list` 등)
- **설정 기반** — 레포별 훅, 파일 복사, 심볼릭 링크
- **모노레포 지원** — 패키지 자동 감지, 패키지별 훅, 포커스 추적
- **헬스 체크** — `oml doctor`로 worktree 문제 진단
- **중앙 집중식 worktree** — 기본적으로 `~/.oml/worktrees/`에 모든 worktree 관리
...
- **템플릿** — 재사용 가능한 worktree 프리셋 (`oml add --template review`)
- **크로스 worktree 실행** — 모든 worktree에서 명령 실행 (`oml exec "bun test"`)
- **GitHub PR 연동** — PR에서 worktree 생성 (`oml add --pr 123`)
- **퍼지 브랜치 피커** — TUI에서 타이핑하면 브랜치 자동완성
- **라이프사이클 관리** — 비활성/머지된 worktree 자동 감지, 제한 설정
- **의존성 공유** — `node_modules` 등을 하드링크/심볼릭으로 디스크 절약
- **Worktree 비교** — worktree 간 변경사항 비교 (`oml diff feature/a feature/b`)
- **핀 보호** — worktree를 자동 정리에서 보호 (`oml pin`)
- **활동 로그** — 생성/삭제/전환/리네임/아카이브/임포트 이벤트 추적 (`oml log`)
- **아카이브** — 제거 전 worktree 변경사항을 패치로 보존 (`oml archive`)
- **브랜치 리네임** — 메타데이터 마이그레이션과 함께 worktree 브랜치 이름 변경 (`oml rename`)
- **클론 및 초기화** — oml 설정과 함께 레포 클론 (`oml clone`)
- **Worktree 임포트** — 수동 생성된 worktree 채택 (`oml import`)
- **상세 뷰** — 커밋 히스토리, diff 통계, upstream 상태 확인 (TUI)
- **일괄 작업** — 다중 선택 및 배치 작업 (TUI)
- **토스트 알림** — 비동기 작업 결과 알림 (TUI)
- **셸 자동완성** — bash/zsh/fish 탭 자동완성 (`oml shell-init --completions`)
- **설정 프로필** — 설정 세트 전환 (`oml config --profiles`)
- **Tmux 세션** — worktree별 tmux 세션 자동 생성/종료, 레이아웃 템플릿 (`oml session`)
- **워크스페이스** — 부모 디렉토리 아래의 git repo를 자동 발견하고 워크스페이스별 defaults 적용 (`workspaces` 설정)
- **AI 에이전트 초기화** — 기본 설정 파일 생성 또는 Claude Code, Codex, OpenCode용 스킬 설치 (`oml init`, `oml init --skill`)
...
- [tmux](https://github.com/tmux/tmux) (선택사항, `oml session`용)
...
`Ctrl+P` → `Open Config`로 엽니다. Config 탭은 `~/.config/oh-my-lemontree/config.json`의 전체 내용을 렌더링합니다:
...
|--------|------|
| `oml` | TUI 실행 |
| `oml list` | 모든 worktree 목록 (포커스 정보 포함) |
| `oml add <branch>` | worktree 생성 |
| `oml remove <branch>` | worktree 제거 |
| `oml switch <branch>` | worktree 전환 |
| `oml clean` | 머지된 worktree 제거 |
| `oml doctor` | worktree 헬스 체크 |
| `oml config` | 설정 관리 |
| `oml exec <command>` | 각 worktree에서 명령 실행 |
| `oml diff <ref1> [ref2]` | worktree/브랜치 간 diff |
| `oml pin <branch>` | worktree 핀/언핀 (정리에서 보호) |
| `oml log` | worktree 활동 로그 보기 |
| `oml archive <branch>` | 변경사항 아카이브 후 선택적 제거 |
| `oml rename <old> <new>` | worktree 브랜치 리네임 |
| `oml clone <url>` | 레포 클론 및 oml 초기화 |
| `oml import <path>` | oml 메타데이터로 worktree 채택 |
| `oml session [branch]` | worktree의 tmux 세션 관리 |
| `oml open [branch]` | worktree를 에디터로 열기 (focus 인식) |
| `oml init`             | 설정 초기화 또는 AI 에이전트 스킬 설치 |

### `oml add`

```bash
oml add feature/login                        # 필요하면 브랜치를 만들고 worktree 생성
oml add feature/login --base main            # 새 브랜치는 main에서 시작
oml add existing-branch                      # 기존 브랜치로 worktree 생성
...
```bash
oml doctor              # 사람이 읽기 쉬운 출력
oml doctor --json       # 스크립팅용 JSON 출력
```
...
```bash
oml list                # Focus 컬럼이 포함된 테이블
oml list --json         # 포커스 배열이 포함된 JSON
oml list --porcelain    # 머신 판독 가능 형식
```
...
```bash
oml remove feature/login               # 브랜치명으로 제거
oml remove feature/login --force        # 강제 제거 (더티 worktree)
oml remove feature/login --yes          # 확인 건너뛰기
```

### `oml clean`

```bash
oml clean --dry-run    # 제거될 항목 미리보기
oml clean              # 머지된 worktree 모두 제거
oml clean --stale      # 비활성 worktree도 표시 (lifecycle 설정 사용)
```
...
```bash
oml exec "bun test"                   # 모든 worktree에서 실행 (순차)
oml exec "bun test" --parallel        # 병렬 실행
oml exec "git pull" --all             # 모든 설정된 레포에서 실행
oml exec "bun install" --dirty        # 더티 worktree만
oml exec "git rebase main" --behind   # upstream보다 뒤처진 것만
oml exec "bun test" --json            # JSON 출력
```
...
커맨드에서 사용 가능한 환경변수: `OML_BRANCH`, `OML_WORKTREE_PATH`, `OML_REPO_PATH`.
...
```bash
oml diff feature/a feature/b         # 전체 diff
oml diff feature/a feature/b --stat  # diffstat 요약
oml diff feature/a --name-only       # 변경된 파일명만
oml diff feature/a                   # 현재 HEAD와 비교
```

### `oml pin`

```bash
oml pin feature/auth --reason "active sprint"  # 사유와 함께 핀
oml pin --list                                  # 핀된 worktree 목록
oml pin --list --json                           # JSON 출력
oml unpin feature/auth                          # 언핀
```

핀된 worktree는 `oml clean` 및 라이프사이클 자동 정리에서 제외됩니다.

### `oml log`

```bash
oml log                # 최근 20개 이벤트 표시
oml log --limit 50     # 최근 50개 이벤트 표시
oml log --json         # JSON 출력
oml log --clear        # 활동 로그 초기화
```
...
```bash
oml archive feature/done --yes       # 아카이브 후 제거
oml archive feature/wip --keep       # 제거 없이 아카이브만
oml archive --list                   # 모든 아카이브 목록
oml archive --list --json            # JSON 출력
```

아카이브는 `~/.oml/archives/`에 패치 파일로 저장됩니다.

### `oml rename`

```bash
oml rename old-branch new-branch             # 브랜치 리네임
oml rename old-branch new-branch --move-path # worktree 디렉토리도 이동
```

### `oml clone`

```bash
oml clone https://github.com/user/repo.git              # 클론 및 초기화
oml clone https://github.com/user/repo.git ./my-dir     # 커스텀 대상 경로
oml clone https://github.com/user/repo.git --template review # 템플릿 적용
oml clone https://github.com/user/repo.git --no-init-config  # 설정 초기화 건너뛰기
```

### `oml import`

```bash
oml import /path/to/worktree                           # worktree 채택
oml import /path/to/worktree --focus apps/web,apps/api # 포커스와 함께
oml import /path/to/worktree --pin                     # 즉시 핀 설정
```
...
```bash
oml session feature/auth              # 세션 열기/연결 (없으면 생성)
oml session feature/auth --layout api # 설정의 레이아웃 사용
oml session --list                    # 활성 oml 세션 목록
oml session --list --json             # JSON 출력
oml session feature/auth --kill       # worktree 세션 종료
oml session --kill-all                # 모든 oml 세션 종료
```
...
`sessions.enabled`가 `true`이고 tmux 안에 있으면, `oml switch`가 대상 worktree의 tmux 세션으로 자동 전환합니다.
...
```bash
oml open                              # 현재 worktree 열기
oml open feature/auth                 # 특정 worktree 열기
oml open feature/auth -e nvim         # 에디터 강제 지정

# Focus 인식 동작 (--focus로 생성된 worktree에서)
oml open feature/auth                 # focus 1개 → 해당 focus 경로로 열림
                                      # focus 2개+ → 에러 + 힌트 출력
oml open feature/auth --focus apps/web   # 특정 focus 경로 선택
oml open feature/auth -f apps/api        # 짧은 별칭
oml open feature/auth --root             # focus 무시하고 worktree 루트 강제

oml open --list-editors               # 감지된 에디터 목록
```
...
기본적으로 oml 설정 파일을 초기화하고, 필요하면 AI 코딩 에이전트용 스킬도 설치합니다.
...
`oml init`을 직접 실행할 필요가 없습니다. 처음으로 `oml` 명령어(또는 TUI)를 실행할 때 `~/.config/oh-my-lemontree/config.json`이 없으면, oml가 기본 설정으로 자동 생성하고 stderr에 한 줄짜리 알림을 출력합니다:

```
oml: created default config at /Users/you/.config/oh-my-lemontree/config.json
```

이 알림은 stdout이 TTY가 아닐 때(파이프, 스크립트, CI 환경)와 `oml init`을 명시적으로 실행할 때(중복 메시지 방지)는 표시되지 않습니다. 자동 초기화는 멱등성을 가지므로 이후 실행에서는 아무 동작도 하지 않습니다.

## 설정

설정 파일: `~/.config/oh-my-lemontree/config.json`

초기화: `oml config --init` (또는 아무 `oml` 명령어 실행 — [첫 실행 시 자동 초기화](#첫-실행-시-자동-초기화) 참조)
...
| 필드 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| `worktreeDir` | `string` | `~/.oml/worktrees/{repo}-{branch}` | worktree 디렉토리 패턴 |
...
사용법: `oml add feature/login --template review`

템플릿 값은 리졸브된 레포 설정을 오버라이드합니다. `base` 필드는 `--base`가 명시적으로 제공되지 않은 경우 기본값을 설정합니다.

#### `lifecycle`

자동 worktree 라이프사이클 관리. `oml clean --stale`에서 사용됩니다.
...
| `autoCreate` | `boolean` | `false` | `oml add` 시 tmux 세션 자동 생성 |
| `autoKill` | `boolean` | `false` | `oml remove` 시 tmux 세션 자동 종료 |
| `prefix` | `string` | `"oml"` | tmux 세션 이름 접두사 |
...
세션 이름 규칙: 브랜치 `feat/auth-token` → tmux 세션 `oml_feat-auth-token`.
...
- `oml list`에서 worktree별 포커스 경로 표시
...
`oml shell-init`으로 `oml switch`용 셸 통합을 설치하세요.
...
```bash
oml config --profiles                    # 프로필 목록
oml config --profile work --activate     # 프로필 활성화
oml config --profile personal --delete   # 프로필 삭제
```
